...
- θ[3] (θ1): Decision threshold for first stage
- θ[4] (θ2): Decision threshold for second stage
- θ[5] (T1): Non-decision time for first stage
- θ[6] (T2): Non-decision time for second stage

Model behavior:
- First stage: Accumulates evidence for all 5 paths (LL, LR, RL, RR, RL) in parallel
...
function model1(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    while true
        ...
        E[1] += (d1 * rewards[1]  + randn()*sigma) + (d1 * rewards[4] + randn()*sigma)
        E[2] += (d1 * rewards[1]  + randn()*sigma) + (d1 * rewards[5] + randn()*sigma)
        E[3] += (d1 * rewards[2]  + randn()*sigma) + (d1 * rewards[6] + randn()*sigma) 
        E[4] += (d1 * rewards[2]  + randn()*sigma) + (d1 * rewards[7] + randn()*sigma)
        E[5] += (d1 * rewards[3]  + randn()*sigma) + (d1 * rewards[8] + randn()*sigma)
        ...
        if v5 > maxv; secv=maxv; maxv=v5; maxidx=5; elseif v5 > secv; secv=v5; end

        if maxv - secv ≥ θ1
            rt1 = t + T1
            if maxidx <= 2
                choice1 = 1
            elseif maxidx > 2 && maxidx <= 4
                choice1 = 2
            elseif maxidx == 5
                choice1 = 3
            end
    ...
    elseif choice1 == 2
        ...
        while true
            ...
            E2[1] += (d2 * rewards[6] + randn()*sigma)
            E2[2] += (d2 * rewards[7] + randn()*sigma)
            
            if abs(E2[1] - E2[2]) ≥ θ2
                rt2    = t2 + T2
                choice2 = E2[1] > E2[2] ? 3 : 4
                break
            end
        end
...
Model behavior:
- First stage: Accumulates evidence with correlated noise between branches
...
function model2(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    # initialize evidence for five paths
    E = zeros(5) # [LL, LR, ML, MR, RL]
    timeout = false
    ...
    while true
        ...
        # update evidence for each path
        E[1] += (d1 * rewards[1]  + noise_L) + (d1 * rewards[4] + randn()*sigma)
        E[2] += (d1 * rewards[1]  + noise_L) + (d1 * rewards[5] + randn()*sigma)
        E[3] += (d1 * rewards[2]  + noise_M) + (d1 * rewards[6] + randn()*sigma)
        E[4] += (d1 * rewards[2]  + noise_M) + (d1 * rewards[7] + randn()*sigma)
        E[5] += (d1 * rewards[3]  + noise_R) + (d1 * rewards[8] + randn()*sigma)
...
Model behavior:
- First stage: Accumulates evidence for all 5 paths in parallel
...
function model3(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    while true
        ...
        # Pruning: if the group_min - outside_max ≥ θ_prun
        if min(E[1], E[2]) - max(E[3], E[4], E[5]) ≥ θ_prun
            rt1 = t + T1
            choice1 = 1  # Left subtree
            break
        elseif min(E[3], E[4]) - max(E[1], E[2], E[5]) ≥ θ_prun
            rt1 = t + T1
            choice1 = 2
            break
        elseif E[5] - max(E[1], E[2], E[3], E[4]) ≥ θ_prun
            rt1 = t + T1
            choice1 = 3
            break
        end
...
function model4(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    # initialize evidence for three paths
    E = zeros(5) # [LL, LR, ML, MR, RL]
    timeout = false
...
function model5(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    while true
        ...
        # update evidence for each path
        E[1] += (d * rewards[1]  + randn()*sigma) + (d * rewards[4] + randn()*sigma)
        E[2] += (d * rewards[1]  + randn()*sigma) + (d * rewards[5] + randn()*sigma)
        E[3] += (d * rewards[2]  + randn()*sigma) + (d * rewards[6] + randn()*sigma)
        E[4] += (d * rewards[2]  + randn()*sigma) + (d * rewards[7] + randn()*sigma)
        E[5] += (d * rewards[3]  + randn()*sigma) + (d * rewards[8] + randn()*sigma)
    ...
    if choice1 == 1
        ...
        while true
            ...
            E2[1] += (d * rewards[4] + randn()*sigma)
            E2[2] += (d * rewards[5] + randn()*sigma)
    ...
    elseif choice1 == 2
        ...
        while true
            ...
            E2[1] += (d * rewards[6] + randn()*sigma)
            E2[2] += (d * rewards[7] + randn()*sigma)
...
Model behavior:
- First stage: Accumulates evidence for three nodes in parallel
- Decision: Made when difference between top two nodes exceeds θ1
- Second stage: Only considers two nodes within chosen side
...
Model behavior:
- Stage 0: Evaluates 5 leaf nodes in parallel
- Stage 1: Uses frozen leaf values to make top-level decision
...
function model7(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    # ---- stage 0 ------#
    # Left‐branch integrators: LL vs LR
    E_LL = 0.0; E_LR = 0.0
    tL = 0
    ...
    # Middle‐branch integrators: ML vs MR
    E_ML = 0.0; E_MR = 0.0
    tM = 0
    while abs(E_ML - E_MR) < θ0
        tM += 1
        E_ML += d0 * rewards[5] + randn()*sigma
        E_MR += d0 * rewards[6] + randn()*sigma
    end

    # Right‐branch integrators: RL
    tR = 0
    
    win_L_val  = max(E_LL, E_LR)
    win_M_val = max(E_ML, E_MR)
    win_R_val = E_RL
    rt0 = max(tL, tM, tR)

    # --- Stage 1: Top‐level competition
    E_top_L = 0.0; E_top_M = 0.0; E_top_R = 0.0
    t1 = 0
    rt1 = 0
    choice1 = 0
    while true
        ...
        E_top_M += d1 * rewards[2] + randn()*sigma
        E_top_R += d1 * rewards[3] + randn()*sigma

        # internal decision variable includes the frozen leaf‐value
        Δ = (E_top_L + win_L_val) - (E_top_M + win_M_val) - (E_top_R + win_R_val)
        if abs(Δ) ≥ θ1
            rt1 = rt0 + t1 + T1
            choice1 = Δ > 0 ? 1 : 2
            break
    ...
    # --- Stage 2: Unfreeze chosen branch (only for left side) ---
    rt2, choice2 = 0.0, 0
    ...
    if choice1 == 2
        ...
        # continue integration for LL vs LR until θ2
        v1, v2 = E_LL, E_LR
        t2 = 0
        while abs(v1 - v2) < θ2
            t2 += 1
            ...
            #     break
            # end
            v1 += d2 * rewards[3] + randn()*sigma  # LL
            v2 += d2 * rewards[4] + randn()*sigma  # LR
        end
        rt2 = t2 + T2
        choice2 = v1 > v2 ? 1 : 2
    end
...
function model8(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    # Right‐branch integrators: only RL
    E_RL = 0.0
    tR = 0
    while abs(E_RL) < θ0
        tR += 1
...
function model9(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    # Determine winning leaf and its value
    winL = (E_LL > E_LR) ? 3 : 4  # LL(3) or LR(4)
    winR = 5  # right winner is always RL
    ...
    if choice1 == 2
        # right chosen: forced RL
        rt2     = T2
        choice2 = 3
    else
        # left chosen: continue LL vs LR
        v1, v2 = E_LL, E_LR
        t2     = 0
        while abs(v1 - v2) < θ2
            t2 += 1
            ...
            #     break
            # end
            v1 += d2 * rewards[3] + randn()*sigma
            v2 += d2 * rewards[4] + randn()*sigma
        end
        rt2     = t2 + T2
        choice2 = (v1 > v2) ? 1 : 2
    end
...
function model11(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    while true
        ...
        # end
        # update evidence for each path
        E[1] += (d * rewards[1]  + randn()*sigma) + (d * rewards[3] + randn()*sigma)  # LL
        E[2] += (d * rewards[1]  + randn()*sigma) + (d * rewards[4] + randn()*sigma)  # LR
        E[3] += (d * rewards[2]  + randn()*sigma) + (d * rewards[5] + randn()*sigma)  # RL
...
function model13(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    while true
        ...
        # end
        # update evidence for each path
        E[1] += (d * noisy_rewards[1]  + randn()*sigma) + (d * noisy_rewards[3] + randn()*sigma)  # LL
        E[2] += (d * noisy_rewards[1]  + randn()*sigma) + (d * noisy_rewards[4] + randn()*sigma)  # LR
        E[3] += (d * noisy_rewards[2]  + randn()*sigma) + (d * noisy_rewards[5] + randn()*sigma)  # RL
...
function model14(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    while true
        ...
        E2[2] += d1 * rewards[4] + η_LR
        E2[3] += d1 * rewards[5] + η_RL

        # accumulate into first stage integrators
        E1[1] += (d1 * rewards[1]  + η_L0) + 0.5 * ((d1 * rewards[3] + η_LL) + (d1 * rewards[4] + η_LR))
        E1[2] += (d1 * rewards[2]  + η_R0) + (d1 * rewards[5] + η_RL)
    ...
    if choice1 == 2
        ...
        while true
            ...
            E2[1] += (d2 * rewards[3] + randn()*sigma)  # LL
            E2[2] += (d2 * rewards[4] + randn()*sigma)  # LR

            if abs(E2[1] - E2[2]) ≥ θ2
                rt2    = t2 + T2
                choice2 = E2[1] > E2[2] ? 1 : 2  # 1=LL, 2=LR
                break
            end
        end
    end
...
# trial = Trial(
#     "test",
#     [4.0, 1.0, 3.0, 2.0, -1.0, 2.0, -3.0, 1.0],
#     1,
#     1,
#     0.0,
#     0.0,

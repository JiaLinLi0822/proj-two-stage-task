...
function add_info(df::DataFrame)
    ...
    df.diff1 = map(path -> round(diff1(path) * 2) / 2, df.path)
...
function summary_stats(df::DataFrame; scale_C=10_000.0, scale_D=1_000.0, scale_E=10_000.0)
    ...
    E_mask = (df.correct1 .== true) .& in.(df.subtree_relation, Ref([1,2,3,4,5]))
...
function simulate_trials(trials, θ, config)
    ...
    @inbounds for i in eachindex(trials)
        ...
        rows[i] = (;
            ...
            choice1 = sim.choice1,
            choice2 = sim.choice2,
            value1 = tr.rewards[1:3],  # Tree3: 3 first-level values
            value2 = tr.rewards[4:8], # Tree3: 5 second-level values
            rewards = tr.rewards,
            path = tr.path
        )
    end
...
@everywhere function fit_model(wid, trials, model_name::String; optimizer::Symbol=:de, NumFuncEvals::Int=5000)
    ...
    function objective_function(x_unit)
        try
            ...
            x = Float64.(x_unit)
            θ_dict = apply(box, x)
            θ = Float64[θ_dict[name] for name in param_names]
    ...
    try
        ...
        real_xopt     = apply(box, xopt_unit)
        xopt = Float64[real_xopt[name] for name in param_names]
...
function main(; data_file::String = "Tree3/data/trial3_v7.json", model_name::String = "model1",
    ...
    if output_file === nothing
        output_file = "Tree3/results/rss/$(model_name)_RSS_$(optimizer)_$(Dates.format(now(), "yyyymmdd_HHMMSS")).csv"
    end
...
# main(; data_file = "Tree3/data/Tree3_v3.json", model_name = "model2", optimizer = :de, NumFuncEvals = 10000)
...
# Adapted for Tree1 structure (5 rewards: R_L, R_R, R_LL, R_RL, R_RR)
...
# ----------------------------- Configuration --------------------------------
const PARTICIPANT_ID = "w4ff13f3"
const TRIAL_INDEX    = 43
const J              = 1000
...
const PLOT_LOG_SPACE = true         # true: plot in log space, false: plot in original space

println("="^80)
println("Participant-Specific KDE Analysis (Tree1)")
println("="^80)
...
# ----------------------------- File paths -----------------------------------
const DATA_FILE       = "Tree1/data/Tree1_v3.json"
const PDA_RESULTS_FILE = "Tree1/results/pda/model6_pda_BADS_20251007_231814.csv"
const IBS_RESULTS_FILE = "Tree1/results/ibs/model6_20250717_032409.csv"
...
function _safe_choice_reward(trial::Trial)
    # Tree1 structure: [R_L, R_R, R_LL, R_LR, R_RR]
    c1r = trial.choice1 == 1 ? trial.rewards[1] : trial.rewards[2]
    c2r = trial.choice1 == 1 ?
          (trial.choice2 == 1 ? trial.rewards[3] : trial.rewards[4]) :  # Left subtree: R_LL, R_LR
          trial.rewards[5]    # Right subtree: R_RR (only one option)
    return c1r, c2r
end
...
function _plot_tree(trial::Trial)
    # Tree1 structure: Left subtree has 2 leaves, Right subtree has 1 leaf
    # Rewards: [R_L, R_R, R_LL, R_LR, R_RR]
    root_x, root_y = 0.5, 0.8
    left_x, left_y = 0.25, 0.4
    right_x, right_y = 0.75, 0.4
    # Tree1 leaves: [R_LL, R_LR, R_RR] - left has 2, right has 1
    leaf = ((0.125,0.1), (0.375,0.1), (0.75,0.1))  # R_LL, R_LR, R_RR
    ...
    plot!(plt, [left_x,  leaf[1][1]],  [left_y,  leaf[1][2]];  color=:black, linewidth=1, label="")  # R_LL
    plot!(plt, [left_x,  leaf[2][1]],  [left_y,  leaf[2][2]];  color=:black, linewidth=1, label="")  # R_LR
    plot!(plt, [right_x, leaf[3][1]],  [right_y, leaf[3][2]];  color=:black, linewidth=1, label="")  # R_RR

    annotate!(plt, left_x  - 0.05, left_y,  text("$(trial.rewards[1])", 8, :right))   # R_L
    annotate!(plt, right_x - 0.05, right_y, text("$(trial.rewards[2])", 8, :right))  # R_R
    annotate!(plt, leaf[1][1] - 0.05, leaf[1][2], text("$(trial.rewards[3])", 8, :right))  # R_LL
    annotate!(plt, leaf[2][1] - 0.05, leaf[2][2], text("$(trial.rewards[4])", 8, :right))  # R_LR
    annotate!(plt, leaf[3][1] - 0.05, leaf[3][2], text("$(trial.rewards[5])", 8, :right))  # R_RR

    if trial.choice1 == 1
        plot!(plt, [root_x, left_x], [root_y, left_y]; color=:red, linewidth=3, label="")
        if trial.choice2 == 1
            plot!(plt, [left_x, leaf[1][1]], [left_y, leaf[1][2]]; color=:red, linewidth=3, label="")  # R_LL
            scatter!(plt, [leaf[1][1]], [leaf[1][2]]; markershape=:circle, ms=6, color=:red, markerstrokewidth=0)
        else
            plot!(plt, [left_x, leaf[2][1]], [left_y, leaf[2][2]]; color=:red, linewidth=3, label="")  # R_LR
        ...
        # Right subtree only has one option (R_RR), so choice2=1 goes to R_RR
        plot!(plt, [right_x, leaf[3][1]], [right_y, leaf[3][2]]; color=:red, linewidth=3, label="")  # R_RR
...
function create_joint_kde_plots(results::Vector, target_trial::Trial)
    ...
    try
        for res in results
            ...
            println("  💾 Attempting to save: $full_path")
...
try
    ...
    try
        # Check if IBS file has wid column, if not skip IBS analysis
        if isfile(IBS_RESULTS_FILE)
            ibs_df = CSV.read(IBS_RESULTS_FILE, DataFrame)
            if "wid" in names(ibs_df)
                ...
                println("⚠️  IBS file doesn't have participant IDs, skipping IBS analysis")
                push!(all_results, nothing)
            end
        else
            println("⚠️  IBS file not found, skipping IBS analysis")
            push!(all_results, nothing)
        end
    catch e
    ...
    random_params = MODEL_NAME == "model1" ? [5e-5, 9e-5, 0.4, 1.1, 700.0, 350.0] :
                   MODEL_NAME == "model3" ? [8e-5, 6e-5, 0.4, 1.1, 700.0, 350.0] :
                   MODEL_NAME == "model6" ? [8e-5, 6e-5, 0.4, 1.1, 700.0, 350.0] :
                   MODEL_NAME == "model7" ? [8e-5, 6e-5, 0.4, 1.1, 700.0, 350.0] :
    ...
    println("\n" * "="^80)
    println("Analysis Summary for Participant $PARTICIPANT_ID, Trial $TRIAL_INDEX (Tree1)")
    println("="^80)
    ...
    println("- Tree1 structure: Left subtree has 1 leaf (R_LL), Right subtree has 2 leaves (R_RL, R_RR)")
    ...
    println("5. Ensure Tree1-specific structure is correctly handled")
end

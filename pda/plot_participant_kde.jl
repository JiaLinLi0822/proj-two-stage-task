...
# ----------------------------- Global plot style -----------------------------
# Match Python matplotlib style: standard single-column width (3.35 x 2.5 inches)
# Font sizes: labelsize=7, titlesize=8, tick labelsize=7, legend fontsize=7
function setup_plot_style!()
    default(
        fontfamily = "Arial",
        guidefont  = font(7, "Arial"),      # axes.labelsize = 7
        tickfont   = font(7, "Arial"),      # xtick.labelsize = 7, ytick.labelsize = 7
        legendfont = font(7, "Arial"),      # legend.fontsize = 7
        titlefont  = font(8, "Arial"),      # axes.titlesize = 8
        framestyle = :axes,
        grid       = false,
        dpi        = 300, 
    )
...
function create_joint_kde_plots(results::Vector, target_trial::Trial)
    ...
    try
        for res in results
            ...
            # Standard single-column width: 3.35 x 2.5 inches at 300 DPI
            # 3.35 inches × 300 DPI = 1005 pixels, 2.5 inches × 300 DPI = 750 pixels
            fig = plot(p_top, tree_plot, p_center, p_right; layout=layout_tpl, size=(1005, 750), margin=5Plots.mm)

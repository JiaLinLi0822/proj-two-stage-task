...
function pda_sampler(trial::Trial, params::Vector{Float64}; 
                                   J::Int=1000, 
                                   max_sims::Int=1000000)
    """
    Generate samples adaptively until we have J samples matching the target choice pair.
    
    Args:
        trial: Target trial with specific choice pair
        params: Model parameters [d1, d2, θ1, θ2, T1, T2]
        J: Desired number of samples for the target choice pair
        max_sims: Maximum number of simulations to prevent infinite loops
    
    Returns:
        matching_samples: Vector of trials matching the target choice pair (length = J)
        total_sims: Total number of simulations performed
    """
    ...
    while matching_sims < J && total_sims < max_sims

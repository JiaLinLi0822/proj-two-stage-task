...
function subtree_vals(v2::AbstractVector{<:Real}, choice1::Int)
    if choice1 == 1
        # Left subtree: indices 1,2 (LL, LR)
        return v2[1:2]
    else
        # Right subtree: index 3 (RL only)
        return [v2[3]]
    end
end
...
function correct2(v2::AbstractVector{<:Real}, choice1::Int, choice2::Int)
    if choice1 == 1
        # Left chosen: choice2 ∈ {1,2} for LL, LR
        c2_local = choice2
        return subtree_vals(v2, choice1)[c2_local] == maximum(subtree_vals(v2, choice1))
    else
        # Right chosen: choice2 = 3 for RL (only one option, always correct if reached)
        return true
    end
end
...
function subtree_relation_code(path::AbstractVector{<:Real})
    ...
    best, second, third = idx_desc
    subtree = i -> (i <= 2 ? 0 : 1)  # 0 for left (LL,LR), 1 for right (RL)
...
function summary_stats(df::DataFrame; scale_C=10_000.0, scale_D=1_000.0, scale_E=10_000.0)
    ...
    # Filter out diff2 == -1 (right subtree with single leaf)
    B_df_filtered = df[df.diff2 .!= -1, :]
    B_df = per_subject_then_group_mean(B_df_filtered; by_cols=[:diff2], y_col=:correct2)

    C_df = per_subject_then_group_mean(df[df.correct1 .== true, :]; by_cols=[:diff1], y_col=:rt1)
    C_df.group_mean .= C_df.group_mean ./ scale_C

    D_df_filtered = df[(df.correct2 .== true) .& (df.diff2 .!= -1), :]
    D_df = per_subject_then_group_mean(D_df_filtered; by_cols=[:diff2], y_col=:rt2)
    D_df.group_mean .= D_df.group_mean ./ scale_D
    
    E_mask = (df.correct1 .== true) .& in.(df.subtree_relation, Ref([1,2]))
...
function simulate_trials(trials, θ, config)
    ...
    @inbounds for i in eachindex(trials)
        ...
        rows[i] = (;
            ...
            value2 = tr.rewards[3:5],  # Tree1: 3 leaf rewards instead of 4
            rewards = tr.rewards,
            path = tr.path
        )
    end
...
function main(; data_file::String = "Tree1/data/Tree1_v3.json", model_name::String = "model1",
    ...
    if output_file === nothing
        output_file = "Tree1/results/rss/$(model_name)_RSS_$(optimizer)_$(Dates.format(now(), "yyyymmdd_HHMMSS")).csv"
    end
    ...
    sim_dir = "Tree1/data/rss"
    if !isdir(sim_dir)
        mkpath(sim_dir)
    end
    json_out = "$sim_dir/$(model_name)_RSS.json"
...
main(; data_file = "Tree1/data/Tree1_v3.json", model_name = "model11", optimizer = :de, NumFuncEvals = 10000)

#!/usr/bin/env julia
# Parameter recovery for full model 6 using IBS (Inverse Binomial Sampling).
# 50 subjects, each with 100 trials randomly generated from model6.
# Fitting uses IBS likelihood and BADS optimizer.
...
function main()
    ...
    println("="^70)
    println("Parameter recovery: Model 6, IBS likelihood")
    println("="^70)
    println("Subjects: $N_SUBJECTS")
    println("Trials per subject: $N_TRIALS_PER_SUBJECT")
    println("Model: $MODEL_NAME")
    println("Likelihood: $LIKELIHOOD_METHOD")
    ...
    out_csv = joinpath(OUTPUT_DIR, "parameter_recovery_model6_ibs_$(timestamp).csv")

#!/usr/bin/env python3
"""
Plot overall rt2 distribution: Human histogram + KDE curves for
Human, TS_RSS, TS_PDA, FG_RSS, FG_PDA (TS = two-stage model1, FG = forward greedy model6).

Usage (from project root):
    python Tree2/plot_rt2_kde_overall.py
Or from Tree2/:
    python plot_rt2_kde_overall.py
"""
...
SOURCES = [
    ("Human", "Human", f"{PREFIX}/Tree2_v3.json"),
...
def main():
    ...
    for key, label, path in SOURCES:
        ...
        df = to_df_rt2(records)
        data[key] = df["rt2"].dropna().values if len(df) > 0 else np.array([])
    ...
    ax.set_ylim(0, None)
    ax.set_xlabel("Second stage RT (ms)", fontsize=14)
    ax.set_ylabel("Density", fontsize=14)
    ax.legend(fontsize=10, ncol=1)
    # ax.grid(False, alpha=0.3)

    plt.tight_layout()
    out_path = "Tree2/figures/Fig5B.svg" if os.path.isdir("Tree2/figures") else "figures/Fig5B.svg"

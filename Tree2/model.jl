...
function model2(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    while true
        ...
        E[2] += (d1 * rewards[1] + noise_L) + (d1 * rewards[4] + randn()*sigma)
...
function model13(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    while true
        ...
        E[2] += (d * noisy_rewards[1]  + randn()*sigma) + (d * noisy_rewards[4] + randn()*sigma)
        E[3] += (d * noisy_rewards[2]  + randn()*sigma) + (d * noisy_rewards[5] + randn()*sigma)
        E[4] += (d * noisy_rewards[2]  + randn()*sigma) + (d * noisy_rewards[6] + randn()*sigma)
...
function model14(θ::Vector{Float64}, rewards::Vector{Float64})
    ...
    while true
        ...
        E2[4] += d1 * rewards[6] + η_RR

        # accumulate into first stage integrators
        E1[1] += (d1 * rewards[1]  + η_L0) + 0.5 * ((d1 * rewards[3] + η_LL) + (d1 * rewards[4] + η_LR))
        E1[2] += (d1 * rewards[2]  + η_R0) + 0.5 * ((d1 * rewards[5] + η_RL) + (d1 * rewards[6] + η_RR))
